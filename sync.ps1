...
try {
    ...
    Write-Host "Verificando migrações e banco de dados..." -ForegroundColor Gray
    
    # Verifica se a CLI do Supabase está disponível
    if (Get-Command supabase -ErrorAction SilentlyContinue) {
        # Empurra migrações locais para o projeto remoto
        try {
        ...
        Write-Error-Custom "CLI do Supabase não encontrada. Pulando esta etapa."
    }
